...
# docker build -t product_register .
# docker run -d -p 80:80 -e RAILS_MASTER_KEY=<value from config/master.key> --name product_register product_register
...
# COPY --from=build /product-register /product-register
...
# ENTRYPOINT ["/product-register/bin/docker-entrypoint"]
